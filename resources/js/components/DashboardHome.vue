<template>
    <section class="py-3 mb-5">
        <span class="d-block mb-5">Home/Dashboard</span>
        <div class="row mb-4">
            <div v-if="lastActivityCourse" class="col-lg-6 mb-5 mb-lg-0">
                <h4 class="mb-4">Recent Lesson</h4>
                <div class="mb-4 theme-shadow course-preview-wrapper">
                    <div class="course-preview"
                        :style="'background-image: url(' + lastActivityCourse?.thumbnail + ');'">
                    ...
                    <div class="p-3">
                        <router-link :to="'/details/' + lastActivityCourse?.id" class="text-decoration-none text-hover">
                            <h5 class="card-title mb-3">{{ lastActivityCourse?.title }}</h5>
                        </router-link>
                        <router-link :to="'/play/' + lastActivityCourse.id"
                            ...
                            <div>
                                ...
                                <small>{{ lastActivityCourse?.title?.length > 30 ? lastActivityCourse?.title.slice(0,
                                    30) + '...' : lastActivityCourse?.title }}</small>
                            </div>
                            <small>{{ formatDuration(lastActivityCourse.total_duration) }}</small>
                        </router-link>
                    </div>
                </div>
            </div>
