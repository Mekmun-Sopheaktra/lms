<template>
    ...
    <section class="bg-light py-5">
        <section class="container">
            <section class="checkout-wizard bg-white theme-shadow my-4">
                <h3 class="fw-bold px-5 py-4 border-bottom mb-0">Checkout</h3>
                <div class="row">
                    <div
                        :class="
                            course?.is_free
                                ? 'col-12'
                                : 'col-8 border-end p-4 ps-5'
                        "
                    >
                        ...
                        <div class="border rounded-3 p-4">
                            ...
                            <div class="bg-light rounded-3 p-4">
                                <div class="price-breakdown mb-3">
                                    ...
                                    <div
                                        class="d-flex justify-content-between mb-4 text-danger"
                                    >
                                        <span>Discount</span>
                                        <strong
                                            >-{{
                                    ...
                                    <div class="mb-3">
                                        <span
                                            ...
                                            >Have any coupon?</span
                                        >
                                    </div>
                                    <form
                                        @submit.prevent="validateCoupon"
                    ...
                    <div
                        class="col-4 ps-0 d-flex flex-column"
                        v-if="!course?.is_free"
                    >
                        <div class="p-4 pe-5">
                            <div
                            ...
                            >
                                ...
                                <small
                                    class="bg-lightgreen text-success rounded px-2 py-1"
                                >
                            ...
                            <div
                                v-for="gateway in masterStore?.masterData
                                    ?.payment_methods"
                                :key="gateway.id"
                            >
                                <div
                                    @click="
                                        selectedPaymentMethod = gateway.gateway
                                    "
                                    :class="
                                        'border d-flex justify-content-between align-items-center px-3 mb-3 rounded payment-method mx-auto' +
                        ...
                        <div class="mt-auto border-top p-4">
                            ...
                            >
                                Pay {{ masterStore?.masterData?.currency_symbol
                                }}{{ grandTotal }}
                                <i class="bi bi-arrow-right float-end"></i>
                            </button>
                        </div>
...
const openPaymentPopupWindow = (url) => {
    ...
    if (win.closed) {
        router.push(
            "/enroll_status?course_id=" + course_id + "&status=" + "cancel"
        );
        return;
    }
    ...
    function trackURLChanges() {
        try {
            ...
            var currentPath = pathname.replace(/\/payment\/\d+/, "");

            var setTime = ref(false);

            if (currentPath.includes("payment/cancel")) {
                win.close();
                clearInterval(intervalID);

                router.push(
                    "/enroll_status?course_id=" +
                        course_id +
                        "&status=" +
                        "cancel"
                );
            ...
            if (currentPath.includes("payment/fail")) {
                win.close();
                clearInterval(intervalID);
                router.push(
                    "/enroll_status?course_id=" +
